  ...
  def content( xhtml : NodeSeq) = {
    ...
    def bindVideoInfo(template: NodeSeq): NodeSeq = {
    	 images.flatMap{ image => 
    	   bind("video_info", template,"id" -> image.id,"lat" -> image.lat,"lng" -> image.lng,"wheather" -> image.wheather,"link" -> <a href={image.video_file_path}><img  width="100" height="100" src={image.thumbnail_file_path}/></a>)   
    	 }
    }
